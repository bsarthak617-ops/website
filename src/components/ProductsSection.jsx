...
function ProductCard({ product, idx, onSelectProductQuote, onOpenSpecs }) {
  ...
  const handleMouseMove = (e) => {
    if (!cardRef.current || window.innerWidth < 1024 || (window.matchMedia && window.matchMedia('(pointer: coarse)').matches)) return;
  ...
  return (
    <motion.div
      ...
      className="group flex flex-col justify-between rounded-[28px] bg-[#F5F4EE] dark:bg-[#181B1A] border border-black/[0.05] dark:border-white/[0.07] p-7 sm:p-8 hover:-translate-y-1 hover:shadow-2xl dark:hover:shadow-black/50 transition-shadow duration-300 [perspective:1000px]"
    >
      ...
      <div className="relative w-full aspect-[16/10] overflow-hidden rounded-2xl border border-black/[0.06] dark:border-white/[0.08] mb-6 sm:mb-8 bg-black/5">
        ...
        />
        <div className="absolute top-3 right-3 px-2 py-0.5 text-[10px] font-mono tracking-wider uppercase bg-black/60 backdrop-blur-md text-white/90 rounded-md border border-white/15 shadow-sm">
          {product.code}
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <button
          ...
          className="w-full text-center font-mono text-[11px] font-semibold text-brand-text-light-subtle hover:text-brand-gold dark:text-brand-text-dark-subtle dark:hover:text-brand-gold tracking-widest uppercase transition-colors py-1.5 flex items-center justify-center gap-1.5 group/specs cursor-pointer"
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
          {PRODUCTS.map((product, idx) => (
            <ProductCard
              ...
              onSelectProductQuote={onSelectProductQuote}
            />
          ))}
        </div>
      </div>
